...
def handle_event(event_data):
    ...
    if event_type == "FileClosed" and relative_path:
        ...
        if os.path.isfile(file_path):
            try:
                subprocess.run(["rclone", "move", file_path, newfile_parent_folder])
                ...
                if os.path.isfile(xml_file_path):
                    subprocess.run(["rclone", "move", xml_file_path, newfile_parent_folder])
                    print(f"[Info] [脚本] [{room_id}] {room_name} 成功移动弹幕文件 {xml_file_path} -> {newfile_parent_folder}")

                if os.path.isfile(jpg_file_path):
                    subprocess.run(["rclone", "move", jpg_file_path, newfile_parent_folder])
