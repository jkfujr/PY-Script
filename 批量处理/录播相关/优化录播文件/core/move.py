...
def move_folder(source, target, enable_move=True):
    if enable_move:
        if not os.path.exists(target):
            logging.info(f"移动文件：{source} -> {target}")
            ...
            for item in os.listdir(source):
                ...
                if os.path.exists(target_item_path):
                    logging.debug(f"目标位置已存在同名项，跳过：{target_item_path}")
                    continue
                logging.debug(f"移动项：{source_item_path} -> {target_item_path}")
                shutil.move(source_item_path, target_item_path)
            try:
                os.rmdir(source)
                logging.debug(f"源文件夹已清空，已删除：{source}")
            except OSError:
                logging.debug(f"源文件夹未完全清空，未删除：{source}")
    else:
